...
execute as @a[scores={tobi_offensive_mode=3}] run title @s actionbar [{"text":"Mode:","color":"white"},{"score":{"name":"@s","objective":"tobi_offensive_mode"},"color":"gold"},{"text":" Armor:","color":"white"},{"score":{"name":"@s","objective":"tobi_has_armor"},"color":"gold"},{"text":" Charge:","color":"white"},{"score":{"name":"@s","objective":"tobi_kamui_charge"},"color":"aqua"}]

# 1. Increase charge if Mode 3 + Sneaking + Survival + Has Armor
execute as @a[scores={tobi_offensive_mode=3,tobi_has_armor=1},predicate=tobi:is_sneaking,gamemode=survival] run scoreboard players add @s tobi_kamui_charge 1
...
execute as @a[scores={tobi_kamui_charge=1..}] unless entity @s[tag=tobi_abilities_enabled] run scoreboard players set @s tobi_kamui_charge 0
