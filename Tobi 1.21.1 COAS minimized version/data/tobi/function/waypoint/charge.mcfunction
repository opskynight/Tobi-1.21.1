...
execute as @a[scores={tobi_charge=1..}] unless entity @s[] run scoreboard players set @s tobi_charge 0

# Action bar feedback while charging (different based on location)
# If in void → charging return
execute as @a[scores={tobi_waypoint=1,tobi_charge=1..99},predicate=tobi:is_sneaking,predicate=tobi:in_kamui_void] run title @s actionbar {"text":"⬤ CHARGING RETURN ⬤","color":"aqua","bold":true}

# If NOT in void → charging warp
execute as @a[scores={tobi_waypoint=1,tobi_charge=1..99},predicate=tobi:is_sneaking] unless predicate tobi:in_kamui_void run title @s actionbar {"text":"⬤ CHARGING WAYPOINT WARP ⬤","color":"dark_purple","bold":true}

# On 100th tick (5 seconds), activate appropriate function
# If in void → return to waypoint
execute as @a[scores={tobi_charge=100..},predicate=tobi:in_kamui_void] run function tobi:waypoint/activate_return

# If NOT in void → warp to void and save location
execute as @a[scores={tobi_charge=100..}] unless predicate tobi:in_kamui_void run function tobi:waypoint/activate_warp
