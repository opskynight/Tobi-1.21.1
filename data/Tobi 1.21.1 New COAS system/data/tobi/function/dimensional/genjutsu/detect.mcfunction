# ============================================
# GENJUTSU - DETECT - FIXED AI RESTORATION
# ============================================
# Properly cleanup when mode changes or stops using

# RESTORE AI when player switches away from mode 2 OR stops holding dimensional COAS
...
execute as @a[scores={tobi_dimensional=0}] at @s as @e[type=!player,type=!item,type=!experience_orb,distance=..15,nbt={NoAI:1b}] run data merge entity @s {NoAI:0b}

# Clear effects when NOT in mode 2 OR not holding dimensional COAS
...
execute as @a[scores={tobi_dimensional=0}] at @s run effect clear @e[distance=..15] minecraft:slowness
execute as @a[scores={tobi_dimensional=0}] at @s run effect clear @e[distance=..15] minecraft:weakness
execute as @a[scores={tobi_dimensional=0}] at @s run effect clear @e[distance=..15] minecraft:blindness
