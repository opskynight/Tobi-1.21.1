...
# ============================================
execute if score @s tobi_offensive_mode matches 3 run clear @s carrot_on_a_stick[custom_data~{tobi_offensive:1b}]
...
# ============================================
# All the if-score branches above read the ORIGINAL score (unchanged).
# This runs LAST so it doesn't cause chain reactions.
# Adds 1 then wraps at 4:  0→1→2→3→0
scoreboard players set #4 tobi_offensive_mode 4
scoreboard players add @s tobi_offensive_mode 1
scoreboard players operation @s tobi_offensive_mode %= #4
