...
# Restore AI when player stops sneaking (but still in mode 0)
...
execute as @a[scores={tobi_offensive_mode=0}] unless predicate tobi:is_sneaking at @s run tag @e[tag=short_range_target,distance=..10] remove short_range_target
