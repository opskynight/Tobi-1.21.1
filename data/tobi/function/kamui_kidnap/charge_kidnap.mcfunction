# Only charge if: slot 3, has armor, NOT on cooldown, sneaking, marker exists
execute as @a[scores={tobi_slot=3,tobi_has_armor=1,tobi_kamui_kidnap_cooldown=0},predicate=tobi:is_sneaking] at @s if entity @e[type=armor_stand,tag=kamui_marker,distance=..25] run scoreboard players add @s tobi_kamui_kidnap_charge 1
...
execute as @a[scores={tobi_kamui_kidnap_charge=40..}] run function tobi:kamui_kidnap/activate